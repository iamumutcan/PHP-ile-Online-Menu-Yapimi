...
if (!empty($_GET["urunid"]))
 {
  ...
  $kontrol=$VT->VeriGetir("urunler","WHERE durum=?",array(1),"ORDER BY ID ASC",1);
    if ($kontrol!=false) 
    {
		...
		if($_POST)
      {
        if(!empty($_POST["ad"]) && !empty($_POST["aciklama"]) && !empty($_POST["durum"]))
        {
          ...
          if(!empty($_FILES["resim"]["name"]))
          {
            ...
            if($yukle!=false)
            {
              $ekle=$VT->SorguCalistir("UPDATE urunler","SET aciklama=?, durum=?, ad=?, resim=? WHERE ID=?",array($aciklama,$durum,$ad,$yukle,$veri[0]["ID"]));
               ?>
          ...
          {	
          	 $ekle=$VT->SorguCalistir("UPDATE urunler","SET aciklama=?, durum=?, ad=? WHERE ID=?",array($aciklama,$durum,$ad,$veri[0]["ID"]));
 ...
 ?>
                     ...
                     <div class="col-12">
                      ...
                      <input type="text" class="form-control" placeholder="Site Url ..." value="<?=stripslashes($veri[0]["ad"])?>" name="ad">
                    </div>
                    <div class="col-12">
                      <label class="form-label">Ürün Açıklama</label>
                      <input type="text" class="form-control" placeholder="Başlık ..." value="<?=stripslashes($veri[0]["aciklama"])?>" name="aciklama">
                    </div>
                    <div class="col-12">
                      <?php 
                      if($veri[0]["durum"]==1)
