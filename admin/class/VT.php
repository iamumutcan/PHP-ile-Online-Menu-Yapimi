...
class VT
{
	...
	public function TabloIsim($EskiAd,$YeniAd)
	{
		
		$this->baglanti->query("SET CHARACTER SET utf8");
		$sql="ALTER TABLE ".$EskiAd." RENAME ".$YeniAd;
		$calistir=$this->baglanti->prepare($sql);
	...
	public function KategoriSil($tabloname){
		$this->baglanti->query("SET CHARACTER SET utf8");
		$sql="DROP TABLE ".$tabloname;
		$calistir=$this->baglanti->prepare($sql);
		$sonuc=$calistir->execute($degerlerarray);
	...
	public function SefLinkYap($tabloAd)
	{	
		$sefAd=str_replace("-", "", $this->seflink($tabloAd));
		return $sefAd;
	}
	public function ModukEkle()
	{
...
');
				    ...
				    $ModukEkle=$this->SorguCalistir("INSERT INTO moduller","SET baslik=?, tablo=?, durum=?, sirano=?, tarih=?",array($baslik,$tablo,$durum,$sirano,date("Y-m-d")));
